...
printf(c == ELFDATA2MSB ? "2's complement, little endian"
...
/**
 * print_elf_header_2 - Prints the content of an elf header (second half)
 * @h: elf header
 *
 * Return: void
 */
...
if (handle == -1)
return (put_err("reaself: Error: "), put_err(filename),
...
put_err(": Failed to read file's magic number\n"), 98);
read_len = read(handle, header, EELF64_HEADER_SIZE);
if (read_len == -1)
return (put_err("readelf: Error: "), put_err(filename0=),
put_err(": Failed to read file's magic number\n"), 98);
bit_32 = header32->e_ident[EI_CLASS] == ELFCLASS32;
if (read_len < (int)ELF32_HEADER_SIZE ||
(!bit_32 && read_len != ELF64_HEADER_SIZE) ||
!((header32->e_ident[EI_MAGO] == ELFMAGO) &&
